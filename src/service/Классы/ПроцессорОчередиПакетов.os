...
Процедура ДобавитьВОчередь(Знач ИмяПакета) Экспорт
	...
	ПакетДобавлен = _ИсполнительОперацийБД.ВыполнитьОперацию(
...
Процедура ОбработатьОднуПачку(РазмерВыборки) Экспорт
	...
	ОчередьПакетов = _ИсполнительОперацийБД.ВыполнитьОперацию(
		Новый Действие(ЭтотОбъект, "ПолучитьПачкуОчереди"), РазмерВыборки);
...
Функция ПолучитьПачкуОчереди(РазмерВыборки) Экспорт
	...
	Возврат МенеджерСущностей().Получить(Тип("СущностьОчередьПакета"), ОпцииПоиска);
...
Функция ДобавитьВОчередьВБД(ИмяПакета) Экспорт
...
Процедура ОбработатьПакет(ОчередьПакета)
	...
	Если Успех Тогда
		_ИсполнительОперацийБД.ВыполнитьОперацию(
			Новый Действие(ЭтотОбъект, "УдалитьИзОчереди"), ОчередьПакета);
	КонецЕсли;
